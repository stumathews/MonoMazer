...
namespace MazerPlatformer
{
    ...
    {
        public static Either<IFailure, Unit> InitializeBounds(Room room) => Statics.Ensure(() =>
        {
			...
			*/
            ...
            var topBounds = new Rectangle(x: room.Rectangle.GetAx(), y: room.Rectangle.GetAy(), width: room.Rectangle.GetAB(), height: 1);
            var bottomBounds = new Rectangle(x: room.Rectangle.GetDx(), y: room.Rectangle.GetDy(), width: room.Rectangle.GetCD(), height: 1);
            var rightBounds = new Rectangle(x: room.Rectangle.GetBx(), y: room.Rectangle.GetBy(), height: room.Rectangle.GetBC(), width: 1);
            var leftBounds = new Rectangle(x: room.Rectangle.GetAx(), y: room.Rectangle.GetAy(), height: room.Rectangle.GetAD(), width: 1);

            /* Walls each have specific colors, bounds, and potentially other configurable characteristics in the game */
            room.AddWallCharacteristic(Room.Side.Top, new SideCharacteristic(Color.Black, topBounds));
            room.AddWallCharacteristic(Room.Side.Right, new SideCharacteristic(Color.Black, rightBounds));
            room.AddWallCharacteristic(Room.Side.Bottom, new SideCharacteristic(Color.Black, bottomBounds));
            room.AddWallCharacteristic(Room.Side.Left, new SideCharacteristic(Color.Black, leftBounds));
        });
